...
pub trait DdmModel: std::fmt::Debug + Send + Sync {
    ...
    fn name(&self) -> &str;
    /// Full description string, matching Java's DDM.toString().
...
/// BEST data: (h², J) pairs from Leal et al. (2012).
/// h² values are (1/d)² where d is Bragg plane spacing; J is expected intensity.
const BEST_DATA: &[(f64, f64)] = &[
...
/// Leal DDM: model from Leal et al. (2012), integrating over resolution shells
/// using BEST expected-intensity data, Debye-Waller factor, and Gaussian scale factor.
/// Parameters gamma, b0, beta correspond to equation 4 in Leal et al. 2012.
#[derive(Debug, Clone)]
pub struct DdmLeal {
    pub gamma: f64,
    pub b0: f64,
    pub beta: f64,
    /// Precomputed: log(h²_mid * J_mid * dh * exp(-0.5 * b0 * h²_mid)) per interval
    log_integral_product: Vec<f64>,
    /// Precomputed: beta * -0.5 * h²_mid per interval
    exponential_product: Vec<f64>,
    /// Integrated intensity at dose = 0
    zero_dose_integrated_intensity: f64,
}

impl DdmLeal {
    pub fn new(gamma: f64, b0: f64, beta: f64) -> Self {
        let n = BEST_DATA.len() - 1;
        let mut log_integral_product = Vec::with_capacity(n);
        let mut exponential_product = Vec::with_capacity(n);

        for i in 0..n {
            let h2_mid = (BEST_DATA[i].0 + BEST_DATA[i + 1].0) / 2.0;
            let j_mid = (BEST_DATA[i].1 + BEST_DATA[i + 1].1) / 2.0;
            let dh = BEST_DATA[i + 1].0.sqrt() - BEST_DATA[i].0.sqrt();

            let lip = (h2_mid * j_mid * dh * (-0.5 * b0 * h2_mid).exp()).ln();
            let ep = beta * -0.5 * h2_mid;

            log_integral_product.push(lip);
            exponential_product.push(ep);
        }

        let zero_dose =
            Self::integrated_intensity(0.0, gamma, &log_integral_product, &exponential_product);

        DdmLeal {
            gamma,
            b0,
            beta,
            log_integral_product,
            exponential_product,
            zero_dose_integrated_intensity: zero_dose,
        }
    }

    fn integrated_intensity(
        dose: f64,
        gamma: f64,
        log_integral_product: &[f64],
        exponential_product: &[f64],
    ) -> f64 {
        let integral_sum: f64 = log_integral_product
            .iter()
            .zip(exponential_product.iter())
            .map(|(&lip, &ep)| (ep * dose + lip).exp())
            .sum();
        (-(gamma * dose).powi(2)).exp() * integral_sum
    }
}

impl DdmModel for DdmLeal {
    fn calc_decay(&self, dose: f64) -> f64 {
        let intensity = Self::integrated_intensity(
            dose,
            self.gamma,
            &self.log_integral_product,
            &self.exponential_product,
        );
        intensity / self.zero_dose_integrated_intensity
    }
    ...
    fn description(&self) -> String {
        format!(
            "Dose Decay Model from Leal et al. 2012 is being used.\nUsing decay parameter values:\n{:>5}{:>2}{:7.3} {}\n{:>5}{:>2}{:7.3} {}\n{:>5}{:>2}{:7.3} {}",
            "Gamma", "=", self.gamma, "MGy^{-1}",
            "B0",    "=", self.b0,    "Angstroms^2",
            "Beta",  "=", self.beta,  "Angstroms^2 MGy^{-1}",
        )
    }
}
...
mod tests {
    ...
    fn ddm_leal_decay_range() {
        let ddm = DdmLeal::new(0.01, 0.0, 0.0);
        let doses = [0.0, 1.0, 5.0, 10.0, 30.0, 43.0, 50.0];
        for &d in &doses {
            let rde = ddm.calc_decay(d);
            assert!(
