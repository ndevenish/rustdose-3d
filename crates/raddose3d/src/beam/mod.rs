...
use crate::parser::config::{BeamConfig, BeamType};
...
pub fn create_beam(config: &BeamConfig) -> Result<Box<dyn Beam>, String> {
    let beam_type = config.beam_type.unwrap_or(BeamType::Gaussian);

    match beam_type {
        BeamType::Gaussian => Ok(Box::new(BeamGaussian::from_config(config)?)),
        BeamType::Tophat => Ok(Box::new(BeamTophat::from_config(config)?)),
        BeamType::Experimental => Ok(Box::new(BeamExperimental::from_config(config)?)),
        BeamType::ExperimentalPgm => {
...
mod tests {
    ...
    fn default_beam_config(beam_type: BeamType) -> BeamConfig {
        BeamConfig {
            beam_type: Some(beam_type),
    ...
    fn create_beam_tophat() {
        let b = create_beam(&default_beam_config(BeamType::Tophat)).unwrap();
    ...
    fn create_beam_gaussian() {
        let b = create_beam(&default_beam_config(BeamType::Gaussian)).unwrap();
