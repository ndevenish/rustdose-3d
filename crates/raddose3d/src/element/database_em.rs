...
impl ElementEM {
    ...
    pub fn elastic_coefficient(&self, electron_energy: f64) -> f64 {
        if electron_energy < 0.05 || electron_energy > 300.0 {
            return 0.0;
        }
    ...
    pub fn atomic_weight(&self) -> f64 { self.atomic_weight }
}
...
impl ElementDatabaseEM {
    fn load() -> Self {
        ...
        for line in ELSEPA_DATA.lines() {
            ...
            for i in 3..max_col {
                let (minus_factor, multiply_factor) = if i >= 23 {
                    (21, 1.0)
                } else {
                    (2, 0.05)
                };
                let energy = (i as f64 - minus_factor as f64) * multiply_factor;
                let val = components[i].trim();
                if !val.is_empty() {
...
mod tests {
    ...
    fn test_em_interpolation() {
        ...
        assert!(xs_exact > 0.0, "Cross section at 0.05 keV should be positive");
        ...
        let (lo, hi) = if xs_low < xs_high { (xs_low, xs_high) } else { (xs_high, xs_low) };
        assert!(xs_interp >= lo && xs_interp <= hi,
            "Interpolated {} should be between {} and {}", xs_interp, lo, hi);
    ...
    fn test_em_hydrogen_values() {
        ...
        assert!((xs - 7.350749e-03).abs() < 1e-8,
            "H at 0.05 keV: expected ~7.35e-3, got {}", xs);
    ...
    fn test_em_iron() {
        ...
        assert!(fe_xs > h_xs, "Fe XS ({}) should exceed H XS ({})", fe_xs, h_xs);
    }
}
